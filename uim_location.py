...
DATA_DIR = r"C:\Users\Jon\Documents\UIUC\CS 538\project\data"
...
def load_wifi_records(user):
    pattern = os.join(DATA_DIR, "uim_exp1_release", user, "btlog", "w*")
...
def load_bt_records(user):
    pattern = os.join(DATA_DIR, "uim_exp1_release", user, "btlog", "s*")
    files = glob.glob(pattern)
    records = []
    for f in files:
        records.extend(load_records_from_file(f))
